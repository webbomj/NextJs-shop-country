      ...
      <div className={styles.top}>
        <div className={styles.img}>
          <Image src={data[0]?.flags?.svg ? data[0]?.flags.svg : data[0]?.flags.png}
            ...
            />
            <div onClick={() => addToCart('comparison')}><Comparison width='30px' height='30px' color='#e86a23'/></div>
        </div>
        <div>
          <h1>{data[0]?.name?.common}</h1>
          <span>{data[0]?.name?.official}</span>
          <div className={styles.href}>Region: <a className={styles.a} onClick={() => router.push(linkRegion)}>{data[0]?.region}</a></div>
