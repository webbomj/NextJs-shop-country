  ...
  return (
    <div className={styles.cart}>
      <div className={styles.img}>
        ...
        <div className={styles.comparison} onClick={() => addToCart('comparison')}><Comparison width='30px' height='30px' color='#e86a23'/></div>
      </div>
      <div className={styles.info}>
        <div className={styles.title}>
          <Link href={`/country/${item.name.common.toLowerCase()}`}><a className={styles.a}><span>{item.name.common.length < 40 ? item.name.common : item.name.common.substring(0, 40) + '...'}</span></a></Link>
        </div>
